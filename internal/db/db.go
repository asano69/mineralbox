...
import (
	"github.com/asano69/mineralbox/internal/errs"
	_ "github.com/asano69/mineralbox/migrations"
...
// PocketBase always needs a directory on disk, so this is mineralbox'
// equivalent of SQLite's ":memory:" mode.
func OpenScratch() (*Database, error) {
	dir, err := os.MkdirTemp("", "mineralbox-pocketbase-*")
	if err != nil {
...
// mineralbox schema exists in it. app is expected to be the single instance
// shared by the whole CLI (see cmd/mineralbox/main.go); its data directory is
// controlled by PocketBase's standard "--dir" flag, not by mineralbox itself.
